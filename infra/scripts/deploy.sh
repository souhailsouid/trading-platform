...
npx ts-node --project tsconfig.cdk.json bin/trading-app.ts deploy TradingStack --require-approval never
...
echo "   aws cloudformation describe-stacks --stack-name TradingStack --query 'Stacks[0].Outputs[?OutputKey==\`WebhookApiUrl\`].OutputValue' --output text"
